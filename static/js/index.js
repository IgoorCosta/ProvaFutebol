...
function ajaxDisponibilizar(){ 
    var filtros = '';
    var filtrosAjax = {
        ope: filtros
    };
    $.ajax({
        url: './pagDisponibilizarIngressos',
        type: 'POST',
        data: {
            filter: JSON.stringify(filtrosAjax)
        },
        success: function(response) {
            var data = JSON.parse(response);
            setTimeout(() => {
                let finddiv = document.getElementById("disponibilizardados");
                finddiv.innerHTML = data.html;
            }, 50);
        }
    });
}

function ajaxComprar(filtros){ 
    var filtrosAjax = {
        ope: filtros
    };
    $.ajax({
        url: './pagComprarIngressos',
        type: 'POST',
        data: {
            filter: JSON.stringify(filtrosAjax)
        },
...
setInterval(ajaxDisponibilizar, 500);